...
# ---------------------------------------------------------------
# Step 1: Get user inputs (supports both interactive and non-interactive modes)
# ---------------------------------------------------------------
# Non-interactive mode: set env vars SERVER_NAME, REPO_URL, JWT_SECRET, SSL_EMAIL
# Interactive mode: script will prompt for each value
if [ ! -t 0 ]; then
    echo -e "${YELLOW}📝 ใช้ค่าจาก Environment Variables (non-interactive mode)${NC}"
    # Non-interactive: read from env vars with defaults
    SERVER_NAME="${SERVER_NAME:-}"
    REPO_URL="${REPO_URL:-https://github.com/tanawinwat-lang/sop-knowledge-base}"
    JWT_SECRET="${JWT_SECRET:-$(openssl rand -hex 32 2>/dev/null || echo "sop-knowledge-base-secret-key-$(date +%s)")}"
    SSL_EMAIL="${SSL_EMAIL:-}"
    if [ -z "$SERVER_NAME" ]; then
        echo -e "${RED}❌ ต้องระบุ SERVER_NAME (ผ่าน env var) สำหรับ non-interactive mode${NC}"
        exit 1
    fi
else
...
if [[ "$SERVER_NAME" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
    ...
    IS_IP=false
    # Interactive SSL email prompt (only when TTY)
    if [ -t 0 ] && [ -z "$SSL_EMAIL" ]; then
